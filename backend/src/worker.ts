...
async function handleJob(job: Job<JobData>, token?: string) {
  ...
  // this covers the case where a job gets retried after the send actually
  // went through but the status update didn't finish before a crash.
  ...
  // rate limit check - per sender, per hour, backed by redis so it stays
  // correct even if we run more than one worker process
